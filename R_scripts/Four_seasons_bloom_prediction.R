...
bias_correction <- readRDS(("bias_corrected_seasonal_forecast_data.rds"))
...
for (j in seq_along(four_seasons_phenoflex_loop)) {
  ...
  for (m in seq_along(four_seasons_phenoflex_loop[[j]])) {
    ...
    for (k in seq_along(four_seasons_phenoflex_loop[[j]][[m]])) {
      ...
      preseason_start <- as.Date(paste0(forecast_year - 1, "-03-01")) # bloom_dates change with this
      season_end <- as.Date(paste0(forecast_year, "-6-30")) # follow the default of genSeason
...
bloom_dates_four_seasons <- readRDS("Bonn_4y7m_seasonal_bloom_dates.rds")
